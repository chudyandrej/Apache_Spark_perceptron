...
class MultiClassPerceptronTest2 extends FlatSpec with Matchers {
  Logger.getLogger("org").setLevel(Level.ERROR)
  "A Multiclass-Perceptron" should "train the data with big accuracy " in {
    ...
    val data_csv = sparkSession.read
      ...
      .option("header", "false")
      .load("./src/universal/multiclass/class6_data.csv")



    val features = data_csv.drop("_c10").drop("_c0").rdd.map(row => {
    ...
    val model = new MultiClassPerceptron(0.002, 500, act_f.sigmoid)
